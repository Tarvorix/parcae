...
THREADS="${THREADS:-4}"
CENTURION_THREADS="${PARCAE_CENTURION_THREADS:-1}"
OUT_TAG="${OUT_TAG:-v1}"
...
# Defaults by profile.
if [[ "$RUN_PROFILE" == "6h" ]]; then
  # Calibrated mid-strength profile (~6-7h on this machine from measured micro-runs).
  BOOK_GAMES="${BOOK_GAMES:-3000}"
  BOOK_MAX_PLIES="${BOOK_MAX_PLIES:-240}"
  BOOK_BOOK_PLIES="${BOOK_BOOK_PLIES:-40}"
  BOOK_EXPLORE_PLIES="${BOOK_EXPLORE_PLIES:-18}"
  BOOK_EPSILON="${BOOK_EPSILON:-0.30}"
  BOOK_PROGRESS_EVERY="${BOOK_PROGRESS_EVERY:-300}"

  TB_GAMES="${TB_GAMES:-5000}"
  TB_MAX_PLIES="${TB_MAX_PLIES:-260}"
  TB_MAX_PIECES="${TB_MAX_PIECES:-4}"
  TB_SYNTHETIC="${TB_SYNTHETIC:-6500}"
  TB_EXPLORE_PLIES="${TB_EXPLORE_PLIES:-24}"
  TB_EPSILON="${TB_EPSILON:-0.40}"
  TB_PROGRESS_EVERY="${TB_PROGRESS_EVERY:-500}"
  TB_PROBE_PROGRESS_EVERY="${TB_PROBE_PROGRESS_EVERY:-500}"

  NNUE_SAMPLES="${NNUE_SAMPLES:-30000}"
  NNUE_EPOCHS="${NNUE_EPOCHS:-8}"
  NNUE_HIDDEN="${NNUE_HIDDEN:-128}"
  NNUE_MAX_PLIES="${NNUE_MAX_PLIES:-160}"
  NNUE_LR="${NNUE_LR:-0.0006}"
  NNUE_EPSILON="${NNUE_EPSILON:-0.35}"
  NNUE_PROGRESS_EVERY="${NNUE_PROGRESS_EVERY:-2000}"
else
  # Full-strength long profile.
...

{
  ...
  echo "centurion_threads=$CENTURION_THREADS"
...
cd "$RUST_DIR"
export PARCAE_CENTURION_THREADS="$CENTURION_THREADS"
